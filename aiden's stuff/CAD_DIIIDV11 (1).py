...
# GEOMETRY
# ============================================================
Stage_0_Model = openmc.DAGMCUniverse(filename='/storage/work/ajg7072/Capstone/Ring_source/cad_modeling/431_GA_Stage_2.7.h5m', auto_geom_ids=True)
...
# ============================================================
R_major  = 187    # cm
z_plasma = 0   # cm — updated for Stage 2.7 (was 119.4 for Stage 0.6)
...
plot.basis = 'xz'
plot.width = (3000, 1184)   # 2*squarecoord2 x 2*VisZcoord2
plot.pixels = (3000, 1184)  # same aspect ratio
plot.origin = (0, 0, 0)
plot.color_by = 'material'
plot.colors = {
    Void: (255, 255, 0),           # yellow — plasma void
    Inconel: (255, 0, 0),          # red — vessel wall
    pfc: (0, 0, 255),              # blue
    GraphiteArmor: (128, 128, 128),# gray
    Air: (200, 200, 255),          # light blue
}
...
img = Image.open('geometry_plot.png')
fig, ax = plt.subplots(figsize=(14, 6))
ax.imshow(img)

# Mark source location — convert cm to pixel coords
# plot goes from x=-250 to x=250 cm, z=24.4-100 to z=24.4+100 cm
# source is at x=187 cm (R_major), z=24.4 cm
src_x_pixel = (187 + 250) / 500 * 2000  # x pixel
src_z_pixel = 800 / 2  # z center pixel
ax.plot(src_x_pixel, src_z_pixel, 'r*', markersize=15, label='Source ring')

patches = [
    mpatches.Patch(color='yellow', label='Plasma void'),
    mpatches.Patch(color='red', label='Inconel'),
    mpatches.Patch(color='blue', label='PFC'),
]
ax.legend(handles=patches, loc='upper right')
plt.savefig('geometry_plot_annotated.png', dpi=150, bbox_inches='tight')
plt.show()
...
# Load statepoint and get flux
sp = openmc.StatePoint('statepoint.20.h5')
flux_data = sp.get_tally(name='flux_3d')

# Use the 2D ntally mesh (1 z slice) for the overlay
# reshape to (nx, ny) for the XZ view
flux_2d = flux_data.mean.reshape((mesh2xy, mesh2xy, mesh2z))
flux_slice = flux_2d[:, 50, :]  # take middle Y slice for XZ view
...
# Show geometry as base
ax.imshow(img, extent=[-squarecoord2, squarecoord2, -VisZcoord2, VisZcoord2],
          aspect='auto', origin='upper')

# Overlay flux with transparency
flux_masked = np.ma.masked_where(flux_slice.T == 0, flux_slice.T)
im = ax.imshow(flux_masked,
               extent=[-squarecoord2, squarecoord2, -VisZcoord2, VisZcoord2],
               origin='lower',
               cmap='inferno',
               norm=mcolors.LogNorm(vmin=flux_masked.min(), vmax=flux_masked.max()),
               alpha=0.6,
               aspect='auto')

plt.colorbar(im, ax=ax, label='Neutron Flux (n/cm²/source-n)')
ax.set_xlabel('X (cm)')
ax.set_ylabel('Z (cm)')
ax.set_title('Flux overlaid on geometry — XZ slice')
plt.savefig('flux_overlay.png', dpi=150, bbox_inches='tight')
plt.show()