...
# Fixes: /meshvol removed, C_mrem indexing corrected, source units fixed,
#        z_plasma updated for Stage 2.7, LowATM/HighATM corrected to air
...
# Stage 2.7 tags: Concrete(0.85)-Barite(0.15), Wall_Shielding,
#   Roof_Shielding, Structural_Steel, CR39, Inconel,
#   nbl, tfc, pfc, LowATM, HighATM, GraphiteArmor
# ============================================================
...
# NOTE: name matches Stage 2.7 H5M tag exactly
Wall_Concrete = openmc.Material.mix_materials(
    [_concrete, _barite], [0.85, 0.15], 'wo',
...
# Inconel — fixed: removed duplicate Ni entry, fractions corrected
...
CR39.add_element("O", 0.190)

GraphiteArmor = openmc.Material(name="GraphiteArmor")
GraphiteArmor.set_density("g/cm3", 2.2)
GraphiteArmor.add_element("C", 1.0)

# LowATM / HighATM — anti-torque coil regions
# These are low-pressure and ambient atmosphere respectively, NOT steel
# Corrected from previous version which used steel composition
LowATM = openmc.Material(name="LowATM")
LowATM.set_density('g/cm3', 1e-4)   # near-vacuum / low pressure
LowATM.add_element('N', 0.78)
LowATM.add_element('O', 0.22)

HighATM = openmc.Material(name="HighATM")
HighATM.set_density('g/cm3', 0.001225)  # ambient air pressure
HighATM.add_element('N', 0.78)
HighATM.add_element('O', 0.22)

mats = openmc.Materials([
    Wall_Concrete, Structural_Steel, Inconel, Roof_Shielding, Wall_Shielding,
    Air, Void, CR39, pfc, tfc, nbl, GraphiteArmor, LowATM, HighATM
])
mats.export_to_xml()
...
# MAGIC WEIGHT WINDOW GENERATION (runs after Pass 1 statepoint)
# The MAGIC method uses the flux tally from Pass 1 to generate
# importance maps that bias particles toward low-flux regions
# (far-field detectors, shielded locations).
# Reference: Griesheimer et al., ANS 2006
def generate_magic_weight_windows(statepoint_file, mesh, output_file='weight_windows.h5'):
    """
    Generate weight windows from a flux tally using the MAGIC method.
    Run this after Pass 1 completes, then set PASS=2 and resubmit.
    """
    sp = openmc.StatePoint(statepoint_file)
    flux_data = sp.get_tally(name="flux_3d")

    # MAGIC: weight = flux_mean / max(flux_mean)
    # Particles in low-flux voxels get higher weights -> more splitting there
    flux_3d = flux_data.mean.reshape((mesh2xy, mesh2xy, mesh2z))
    flux_max = flux_3d.max()
    flux_3d_safe = np.where(flux_3d > 0, flux_3d, flux_max * 1e-10)

    # Weight window bounds: lower = 0.5 * (flux/max), upper = 2.5 * lower
    ww_lower = 0.5 * (flux_3d_safe / flux_max)
    ww_upper = 2.5 * ww_lower
    
    wws = openmc.WeightWindows(
        mesh=mesh2,
        lower_ww_bounds=ww_lower.flatten(),
        upper_ww_bounds=ww_upper.flatten(),
        particle_type='neutron',
    )
    openmc.wws_to_hdf5([wws], output_file)

    print(f"MAGIC weight windows saved to {output_file}")
    print(f"  Flux range: {flux_3d[flux_3d>0].min():.2e} to {flux_max:.2e}")
    print(f"  WW lower range: {ww_lower.min():.2e} to {ww_lower.max():.2e}")
    return wws
...
if PASS == 1:
    ...
    print("\nPass 1 complete. Set PASS=2 and resubmit for production run.")
