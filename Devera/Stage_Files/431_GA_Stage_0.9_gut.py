...
mats = openmc.Materials([Wall_Concrete,Structural_Steel,Inconel,Roof_Shielding,Wall_Shielding,Air,Void,CR39])
...
Stage_0_Model = openmc.DAGMCUniverse(filename = '/bin/431_GA_Stage_0.9.h5m', auto_geom_ids = True)
...
settings.batches = 22 # oriignally 100 but for testing we shouldnd't need this many.
...
mesh.dimension = (squarecoord,1,squarecoord) # I want the mesh to have to have: 
...
implicit_comp_filter = openmc.CellFilter([1])
...
sp = openmc.StatePoint("statepoint.22.h5") # Note to self: state point 
# number (statepoint.xxx.h5) xxx is NOT ARBITRARY AND IS LINKED TO THE NUMBER OF BATCHES
# CHANGE THIS DO NOT FORGET NIGHTMARE NIGHTMARE NIGHTMARE. (i wasted 30 minutes of my time on this)
