...
pfc.add_element("C", 0.324)
pfc.add_element("H", 0.486)
pfc.add_element("O", 0.190)
# TFCs:
tfc = openmc.Material(name="tfc")
tfc.set_density("g/cm3", 1.31)
# CR39 element proportions based on single polymer structure (repeated to form the actual chain)
tfc.add_element("C", 0.324)
tfc.add_element("H", 0.486)
tfc.add_element("O", 0.190)

# NB: 
nbl = openmc.Material(name="nbl")
nbl.set_density("g/cm3", 1.31)
# CR39 element proportions based on single polymer structure (repeated to form the actual chain)
nbl.add_element("C", 0.324)
nbl.add_element("H", 0.486)
nbl.add_element("O", 0.190)
...
mats = openmc.Materials([Wall_Concrete,Structural_Steel,Inconel,Roof_Shielding,Wall_Shielding,Air,Void,CR39, pfc, tfc, nbl])
...
Stage_0_Model = openmc.DAGMCUniverse(filename = '/bin/431_GA_Stage_1.5.h5m', auto_geom_ids = True)
...
VisMesh = 80 # Controls both the visualization tallies and the dose calculations.
...
detectors = {
    # Name                   xyz (cm) (tbd)            FN_mrem(from report)   Notes
    'Locator':          ((0,     600,     0),    30,      'Transrex area, far wall - XZ centered'),
}
...
detectors5 = {
    ...
    'EXP_9_NorthDoor_Non_LOS': ((-874.4,  940,     00),    860,     'Lower N door WITH line-of-sight to vessel'),
    'EXP_10_NorthDoor_LOS':    ((-582.9,  940,     00),    8910,    'Lower N door WITHOUT vessel view — verify vs EXP_9'),
    'EXP_11_DeyongOut':        ((-557.2, 487.6,    80),    5060,    'Outside Deyong shield box'),
    'EXP_12_DeyongIn':         ((-557.2, 487.6,    40),    None,    'Inside Deyong box — FN exceeded detector cap'),
    'EXP_13_150V+1':           (( 50,      50,   200),    6620,    '150V+1 near vessel'),
    'EXP_14_150V-1':           (( 50,       50,  -200),    19540,   '150V-1 near vessel — highest FN'),
    'EXP_15':                  (( 200,   557.2,     0),    18210,   'Second highest FN location'),
} # Modified detector locations slightly
...
slice_value = 45 # <- manual override of slice value 
dose_mrem_3dlog = np.log(dose_mrem_3d)  # dose_mrem_3d <- for just using raw linear input 
dose_plot_data = dose_mrem_3d[:,:,slice_value] # change to log or non-log for plotting purposes.
